...
const concurrency = 1 // Make it 1, we just want it to be successful don't care about speed.
...
const generateScripts = async (
  ...
  const contentTypeResponse = await environment.getContentTypes({
    limit: 300 // never gonna be 1000 content-types, so just set to 300
  })
  let requiredContentTypes = contentTypeResponse.items.filter(
    (item) => item.sys.id !== 'migration'
  )
  if (contentTypes.length > 0) {
    const contentTypesSet = new Set(contentTypes)
    requiredContentTypes = requiredContentTypes.filter((contentType) =>
      contentTypesSet.has(contentType.sys.id)
    )
  }
  ...
  log(
    chalk.bold.green('🎉  Scripts generation'),
